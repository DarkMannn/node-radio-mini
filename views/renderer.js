...
class Playlist {
    ...
    constructor() {
        ...
        this.eventHandlers = new KeyListenerClass({
            box: this.box,
            actionFn: Utils.pipe(Utils.pick('content'), Utils.discardFirstWord, exports.createChildAndAppendToQueue),
            bgPlain: bgPlPlain,
            bgFocus: bgPlFocus
        });
        this.eventHandlers.sendToQueueWindow = this.eventHandlers.action;
    ...
    _createBoxChildAndAppendToBox(content) {
    ...
    _fillWithItems(items) {
        ...
        for (const item of items) {
            this._createBoxChildAndAppendToBox(item);
        }
    }
